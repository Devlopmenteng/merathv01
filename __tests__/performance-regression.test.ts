...
describe('Performance Regression Tests', () => {
  ...
  TEST_CASES.forEach(({ name, estate, heirs, maxDuration }) => {
    it(`${name} should complete within threshold`, () => {
      ...
      console.log(
        `[Performance] ${name}: ${avgDuration.toFixed(2)}ms (threshold: ${maxDuration}ms)`
      );

      if (!passed) {
        console.warn(
          `[Performance Regression] ${name} exceeded threshold by ${((avgDuration / maxDuration - 1) * 100).toFixed(0)}%`
        );
      }
  ...
  it('cache hit should be significantly faster than cache miss', async () => {
    ...
    console.log(
      `[Performance] Cold: ${coldDuration.toFixed(2)}ms, Warm: ${warmDuration.toFixed(2)}ms`
    );
