...
export async function generateLegalReport(result: any, madhab: string) {
  ...
  // Simple concatenation instead of interpolation
  const headerTitle = t('report_header');
  const subtitle = t('report_subheader') + ' ' + madhab + ' – ' + today;
  const caseNumLabel = t('report_case_number') + ' ' + caseNumber;
  const estateSummary = t('report_estate_summary');
  const netEstateLabel = t('report_net_estate');
  const distributionTitle = t('report_distribution');
  const generatedBy = t('report_generated_by');
  const contact = t('report_contact');
  const disclaimerText = t('report_disclaimer');
  ...
  <body>
    ...
    <div class="section">
      <h3>${estateSummary}</h3>
      <p>${netEstateLabel}: ${formatCurrency(result.netEstate || result.netTotal)}</p>
    </div>
    <div class="section">
      ...
      <table class="table">
        ...
        <tbody>
          ${result.shares.map((s: any) => `
            <tr><td>${s.name}</td><td>${formatCurrency(s.amount)}</td><td>${s.fraction.numerator}/${s.fraction.denominator}</td></tr>
          `).join('')}
        </tbody>
      </table>
    </div>
