  ...
  const generatePDF = async () => {
    ...
    const html = `
      ...
      <body>
        ...
        <p>Madhab: ${escapeHtml(String(result.madhab))}</p>
        <p>Net Estate: $${Number(result.netEstate)}</p>
        <h2>Distribution</h2>
        <ul>${result.shares
          .map(
            (s) =>
              `<li>${escapeHtml(String(s.name))}: $${Number(s.amount).toFixed(2)} (${Number(s.fraction?.numerator)}/${Number(s.fraction?.denominator)})</li>`,
          )
          .join('')}</ul>
