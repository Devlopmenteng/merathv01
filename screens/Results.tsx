...
export const Results = ({ navigation }: { navigation: any }) => {
  ...
  const chartData = useMemo<ChartDataItem[]>(() => {
    ...
    return result.shares.map((s, idx) => ({
      ...
      fraction: formatFraction(s.fraction),
    }));
  }, [result]);
    ...
    const html = `
      ...
      <body>
        ...
        <ul>${result.shares
          .map(
            (s) =>
              `<li>${s.name}: $${s.amount.toFixed(2)} (${formatFraction(s.fraction)})</li>`,
          )
          .join('')}</ul>
  ...
  const copyAsText = async () => {
    ...
    result.shares.forEach((s) => {
      text += `${s.name}: $${s.amount.toFixed(2)} (${formatFraction(s.fraction)})\n`;
    });
            ...
            {result.shares.map((share, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={{ width: 80, textAlign: "center" }}>{formatFraction(share.fraction)}</Text>
