...
export const Settings = ({ navigation }: { navigation: SettingsNavigation }) => {
  ...
  const handleRateUs = () => {
    // Replace with actual store link when published
    Linking.openURL('https://play.google.com/store/apps/details?id=your.app.id');
  };

  const handlePrivacyPolicy = () => {
    Linking.openURL('https://your-privacy-policy-url.com'); // <-- Replace with actual URL
  };
    ...
    Alert.alert(
      ...
      [
        { text: t('cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              // Clear all AsyncStorage data (excluding maybe user preferences if any)
              await AsyncStorage.clear();
              showAlert(t('success'), t('cache_cleared_restart'));
              // Optionally restart the app or just reload the screen
            } catch (error) {
